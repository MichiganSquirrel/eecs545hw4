...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        # Step 1： compute the initial hidden state from the image features (N, D) -> (N, H)
        h0 = features @ W_proj + b_proj
        # Step 2： word embedding layer to transform the words in captions_in from indices to vectors (N, T-1) -> (N, T-1, W)
        x, cache_embed = word_embedding_forward(captions_in, W_embed)
        # Step 3： vanilla RNN to process the sequence of input word vectors (T, N, W) -> (T-1, N, H)
        x = x.transpose(1, 0, 2)
        h, cache_rnn = rnn_forward(x, h0, Wx, Wh, b)
        # Step 4： compute scores over the vocabulary at every timestep using the hidden states (N, T-1, V)
        h = h.transpose(1, 0, 2)  # Convert from (T-1, N, H) to (N, T-1, H)
        scores, cache_temporal_fc = temporal_fc_forward(h, W_vocab, b_vocab)
        # Step 5： compute loss using captions_out
        loss, dscores = temporal_softmax_loss(scores, captions_out, mask)


        # backward pass
        # back from softmax layer
        dh, dW_vocab, db_vocab = temporal_fc_backward(dscores, cache_temporal_fc)
        ...
        # back from embedding layer
        dx = dx.transpose(1, 0, 2)
        dW_embed = word_embedding_backward(dx, cache_embed)
        grads['W_embed'] = dW_embed

        # back from projection layer
        dW_proj = features.T @ dh0  # (D, N) @ (N, H) -> (D, H)
        db_proj = np.sum(dh0, axis=0)  # Sum across batch dimension (N, H) -> (H,)
