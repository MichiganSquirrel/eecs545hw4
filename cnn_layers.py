...
def conv_forward(x, w):
    ...
    ###########################################################################
    # N,C stands for number of images and number of channels
    # F,C stands for number of filters and number of channels
    #! 广播机制
    for j in range(0, H_prime):
        for i in range(0, W_prime):
            #get the slice of X
            # shape of x_slice = (N, C, HH, WW)
            x_slice = x[:, :, j:j + HH, i:i + WW]
            # shape of out = (N, F, H', W')
            out[:, :, j, i] = np.sum(
                x_slice[:, None, :, :, :] * w[None, :, :, :, :],
                axis=(2, 3, 4)
            )
...
def conv_backward(dout, cache):
    ...
    ###########################################################################
    # # flip the derivative of the upstream
    # new_dout = dout[: ,: , None, :, :]
    # new_dw = dw[None, :, :, :, :]
    # for j in range(H):
    #     for i in range(W):
    #         dx[:, :, j, i] = np.sum(new_dout * new_dw, axis = (1,3,4))

    w_flip = w[:, :, ::-1, ::-1]
    dout_pad = np.pad(dout, ((0,0), (0,0), (HH-1, HH-1), (WW-1, WW-1)),
                  mode='constant', constant_values=0)
    
    for i in range(H):
        for j in range(W):
          # 从 dout_pad 中提取对应的局部区域，形状为 (N, F, HH, WW)
          dout_slice = dout_pad[:, :, j:j+HH, i:i+WW]
          # 利用广播机制进行逐元素乘法：
          #  - 将 dout_slice 维度扩展为 (N, F, 1, HH, WW)
          #  - 将 w_flip 维度扩展为 (1, F, C, HH, WW)
          # 这样相乘得到的数组形状为 (N, F, C, HH, WW)
          # 随后在 (F, HH, WW) 这三个维度上求和，得到形状 (N, C)
          dx[:, :, j, i] = np.sum(dout_slice[:, :, None, :, :] * w_flip[None, :, :, :, :], axis=(1, 3, 4))

    for j in range(HH):
        for i in range(WW):
          # 取出对应的 dout 切片: 形状 (N, F, H_prime, W_prime)
          # 取出对应的 x 切片: 形状 (N, C, H_prime, W_prime)
          x_slice = x[:, :, j:j + H_prime, i:i + W_prime]
          # 在 x_slice 上增加一个新的维度，使得形状变成 (N, 1, C, H_prime, W_prime)
          # 这样 dout_slice 和 x_slice 就可以广播运算：
          # dout_slice: (N, F, H_prime, W_prime) → (N, F, 1, H_prime, W_prime)
          # x_slice:  (N, C, H_prime, W_prime) → (N, 1, C, H_prime, W_prime)
          # 相乘得到 (N, F, C, H_prime, W_prime)
          dw[:, :, j, i] = np.sum(dout[:, :, None, :, :] * x_slice[:, None, :, :, :], axis=(0, 3, 4))
